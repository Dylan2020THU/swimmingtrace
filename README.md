...
**会员洞察**：owner 控制台的会员表加 **性别 / 年龄组 / 状态** 信息列、**筛选** 与逐行「**个人泳迹图**」入口（`/swimmers/:sid` 只读热力图，复用 `GET /stats/swimmer/:sid`）。按表量体裁衣：**挑战排行榜** 额外含 游泳次数 / 里程占比，客户端筛选；**泳池名册** 走服务端筛选 `?gender=&status=&q=`；**赛事报名席** 加 年龄组（按赛事日期计龄）+ 泳迹图按钮。年龄组档位与计龄函数（`ageGroupOf`）提升至 `@swim/shared`，赛事/会员两侧口径统一。设计见 [`docs/superpowers/specs/2026-06-30-member-insights-design.md`](docs/superpowers/specs/2026-06-30-member-insights-design.md)。
