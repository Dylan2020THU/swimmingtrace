...
| `GET /pools/:id/swimmers` · `POST /pools/:id/swimmers` | 名册（**分页** `?page=&pageSize=` + **筛选** `?gender=&status=&q=`，返回 `{items,total,page,pageSize}`）/ 新建会员（邮箱已存在则复用） |
...
**会员洞察**：owner 控制台的会员表加 **性别 / 年龄组 / 状态** 信息列、**筛选** 与逐行「**个人主页**」入口（`/swimmers/:sid`）。该页为 **GitHub 个人主页式会员页**：左栏个人基本信息卡（头像/姓名/邮箱/性别·年龄组·认领状态 + 所属泳池列表），右栏训练概览（全期里程/次数/时长）+ 活动热力图（**年份选择器**）+ 训练明细（按月倒序分组，对标 GitHub contribution activity，加载更多）。新增 owner-only 端点 `GET /stats/swimmer/:sid/profile`、`/sessions?year=&page=`（`assertOwnsSwimmer`，**仅本人名下泳池**作用域）。按表量体裁衣：**挑战排行榜** 额外含 游泳次数 / 里程占比，客户端筛选；**泳池名册** 走服务端筛选 `?gender=&status=&q=`；**赛事报名席** 加 年龄组（按赛事日期计龄）+ 泳迹图按钮。年龄组档位与计龄函数（`ageGroupOf`）提升至 `@swim/shared`，赛事/会员两侧口径统一。设计见 [`docs/superpowers/specs/2026-06-30-member-insights-design.md`](docs/superpowers/specs/2026-06-30-member-insights-design.md)。
...
排名**按 (性别 × 年龄组) 分组**（标准年龄组：6至8岁 / 9至14岁 / 15至18岁 / 19至35岁 / 36至45岁 / 46至55岁 / 56至69岁 / 70岁以上，按赛事日期计龄）：组内 `OK` 完赛者按时间升序、并列同名次、每组前三发**金/银/铜**；DNS/DNF/DQ 不计名次。成绩存毫秒整数（`parse/formatSwimTime`）。创建赛事为 **Pro 功能**（Free → `402`）。名次/年龄组为纯函数，重点单测覆盖。设计见 [`docs/superpowers/specs/2026-06-30-meets-e1-design.md`](docs/superpowers/specs/2026-06-30-meets-e1-design.md)。
