...
const mkBilling = () =>
  ({ assertCanCreatePool: jest.fn().mockResolvedValue(undefined), assertCanAddMember: jest.fn().mockResolvedValue(undefined) }) as any;
...
describe('PoolsService.generateClaimLink', () => {
  ...
  it('未认领 SWIMMER → 写入随机令牌+7天过期，返回 claimUrl', async () => {
    ...
    const mail = mkMail();
    const svc = new PoolsService(prisma, { get: () => undefined } as any, mail, mkBilling());
...
describe('PoolsService 计划配额强制', () => {
  it('createPool 先查泳池配额，超限即抛、不建池', async () => {
    const prisma: any = { pool: { create: jest.fn() } };
    const billing = { assertCanCreatePool: jest.fn().mockRejectedValue(new Error('quota')), assertCanAddMember: jest.fn() } as any;
    const svc = new PoolsService(prisma, { get: () => undefined } as any, mkMail(), billing);
    await expect(svc.createPool('o1', { name: 'P' } as any)).rejects.toThrow('quota');
    expect(billing.assertCanCreatePool).toHaveBeenCalledWith('o1');
    expect(prisma.pool.create).not.toHaveBeenCalled();
  });

  it('createSwimmer 先查会员配额，超限即抛、不建会员', async () => {
    const prisma: any = {
      pool: { findUnique: jest.fn().mockResolvedValue({ id: 'p1', ownerId: 'o1' }) },
      user: { findUnique: jest.fn() },
    };
    const billing = { assertCanCreatePool: jest.fn(), assertCanAddMember: jest.fn().mockRejectedValue(new Error('quota')) } as any;
    const svc = new PoolsService(prisma, { get: () => undefined } as any, mkMail(), billing);
    await expect(svc.createSwimmer('o1', 'p1', { email: 'a@b.c' } as any)).rejects.toThrow('quota');
    expect(billing.assertCanAddMember).toHaveBeenCalledWith('o1');
