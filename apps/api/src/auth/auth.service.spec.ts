...
describe('AuthService.register', () => {
  it('邮箱已存在 → ConflictException', async () => {
    ...
    const svc = new AuthService(prisma, mkJwt(), mkRefresh());
...
describe('AuthService.login', () => {
  ...
  it('密码正确 → 返回 accessToken（payload 含 sub/email/role）', async () => {
    ...
    const prisma: any = { user: { findUnique: jest.fn().mockResolvedValue({ id: 'u1', email: 'a@b.c', role: 'OWNER', passwordHash: 'H' }) } };
    const jwt = mkJwt();
    const svc = new AuthService(prisma, jwt, mkRefresh());
...
describe('AuthService.getClaimInfo', () => {
  it('有效令牌 → 返回 name/email', async () => {
    ...
    await expect(new AuthService(prisma, mkJwt(), mkRefresh()).getClaimInfo('tok')).resolves.toEqual({ name: 'Sam', email: 's@x.c' });
  });
  it('令牌不存在 → NotFound', async () => {
    const prisma: any = { user: { findUnique: jest.fn().mockResolvedValue(null) } };
    await expect(new AuthService(prisma, mkJwt(), mkRefresh()).getClaimInfo('x')).rejects.toBeInstanceOf(NotFoundException);
  });
  it('过期 → Gone', async () => {
    ...
    await expect(new AuthService(prisma, mkJwt(), mkRefresh()).getClaimInfo('tok')).rejects.toBeInstanceOf(GoneException);
  });
  it('已认领 → Conflict', async () => {
    ...
    await expect(new AuthService(prisma, mkJwt(), mkRefresh()).getClaimInfo('tok')).rejects.toBeInstanceOf(ConflictException);
  });
});
...
describe('AuthService.claim', () => {
  it('有效 → 写 pw/claimedAt、清令牌、签发 token', async () => {
    ...
    const jwt = mkJwt();
    const res = await new AuthService(prisma, jwt, mkRefresh()).claim({ token: 'tok', password: 'password123' });
