...
const mkRefresh = () =>
  ({ issue: jest.fn().mockResolvedValue('refresh.tok'), rotate: jest.fn(), revoke: jest.fn(), revokeAllForUser: jest.fn() }) as any;
const mkEmailVerify = () =>
  ({ sendVerification: jest.fn().mockResolvedValue(undefined), verify: jest.fn(), resend: jest.fn() }) as any;

describe('AuthService.register', () => {
  it('邮箱已存在 → ConflictException', async () => {
    const prisma: any = { user: { findUnique: jest.fn().mockResolvedValue({ id: 'u1' }) } };
    const svc = new AuthService(prisma, mkJwt(), mkRefresh(), mkEmailVerify());
  ...
  it('新用户 → bcrypt 哈希、创建并签发 token；role 默认 SWIMMER', async () => {
    ...
    expect(res).toEqual({ accessToken: 'signed.jwt.token', refreshToken: 'refresh.tok' });
  });
...
describe('AuthService.login', () => {
  ...
  it('密码正确 → 返回 accessToken（payload 含 sub/email/role）', async () => {
    ...
    const prisma: any = { user: { findUnique: jest.fn().mockResolvedValue({ id: 'u1', email: 'a@b.c', role: 'OWNER', passwordHash: 'H' }) } };
    const jwt = mkJwt();
    const svc = new AuthService(prisma, jwt, mkRefresh(), mkEmailVerify());
...
describe('AuthService.getClaimInfo', () => {
  it('有效令牌 → 返回 name/email', async () => {
    ...
    await expect(new AuthService(prisma, mkJwt(), mkRefresh(), mkEmailVerify()).getClaimInfo('tok')).resolves.toEqual({ name: 'Sam', email: 's@x.c' });
  });
  it('令牌不存在 → NotFound', async () => {
    const prisma: any = { user: { findUnique: jest.fn().mockResolvedValue(null) } };
    await expect(new AuthService(prisma, mkJwt(), mkRefresh(), mkEmailVerify()).getClaimInfo('x')).rejects.toBeInstanceOf(NotFoundException);
  });
  it('过期 → Gone', async () => {
    ...
    await expect(new AuthService(prisma, mkJwt(), mkRefresh(), mkEmailVerify()).getClaimInfo('tok')).rejects.toBeInstanceOf(GoneException);
  });
  it('已认领 → Conflict', async () => {
    ...
    await expect(new AuthService(prisma, mkJwt(), mkRefresh(), mkEmailVerify()).getClaimInfo('tok')).rejects.toBeInstanceOf(ConflictException);
  });
});
...
describe('AuthService.claim', () => {
  it('有效 → 写 pw/claimedAt、清令牌、签发 token', async () => {
    ...
    const jwt = mkJwt();
    const res = await new AuthService(prisma, jwt, mkRefresh(), mkEmailVerify()).claim({ token: 'tok', password: 'password123' });
