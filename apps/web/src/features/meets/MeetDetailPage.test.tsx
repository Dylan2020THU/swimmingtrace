  ...
  server.use(
    http.get('/api/meets/m1', () =>
      HttpResponse.json({
        ...
        hostPoolId: null, hostPoolName: null, laneCount: 6, eventCount: 1, published: false, registrationOpen: false, createdAt: '2026-06-30T00:00:00.000Z',
...
it('「开放报名」开关触发 POST /meets/:id/registration', async () => {
  ...
  server.use(
    http.get('/api/meets/m1', () =>
      HttpResponse.json({
        id: 'm1', name: '报名赛', meetDate: '2026-06-30T00:00:00.000Z',
        hostPoolId: null, hostPoolName: null, laneCount: 6, eventCount: 0, published: false, registrationOpen: false, createdAt: '2026-06-30T00:00:00.000Z',
        events: [],
      })),
    http.post('/api/meets/m1/registration', async ({ request }) => { body = await request.json(); return HttpResponse.json({ registrationOpen: true }); }),
  );

  renderWithProviders(<Routes><Route path="/meets/:meetId" element={<MeetDetailPage />} /></Routes>, { route: '/meets/m1' });
  await screen.findByText('报名赛');
  const switches = screen.getAllByRole('switch');
  await userEvent.click(switches[0]); // 开放报名 is the first switch
  await waitFor(() => expect(body).toEqual({ registrationOpen: true }));
});
